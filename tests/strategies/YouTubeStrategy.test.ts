...
describe('YouTubeStrategy', () => {
  ...
  describe('canHandle', () => {
    it('should handle standard watch URLs', () => {
      expect(
        strategy.canHandle('https://www.youtube.com/watch?v=dQw4w9WgXcQ'),
      ).toBe(true);
      expect(
        strategy.canHandle('https://youtube.com/watch?v=dQw4w9WgXcQ'),
      ).toBe(true);
    });

    it('should handle mobile watch URLs', () => {
      expect(
        strategy.canHandle('https://m.youtube.com/watch?v=dQw4w9WgXcQ'),
      ).toBe(true);
    });

    it('should handle music watch URLs', () => {
      expect(
        strategy.canHandle('https://music.youtube.com/watch?v=dQw4w9WgXcQ'),
      ).toBe(true);
    });

    it('should handle YouTube Shorts', () => {
      expect(strategy.canHandle('https://www.youtube.com/shorts/abcd123')).toBe(
        true,
      );
    });
    ...
    it('should NOT handle YouTube home or other pages', () => {
      ...
      expect(
        strategy.canHandle('https://www.youtube.com/feed/subscriptions'),
      ).toBe(false);
    });
