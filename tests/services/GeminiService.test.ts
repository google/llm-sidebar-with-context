  ...
  it('should handle multimodal context (e.g. YouTube)', async () => {
    ...
    const context = [
      { type: 'text' as const, text: 'YouTube Video Header' },
      {
        ...
        mimeType: 'video/mp4',
        fileUri: 'https://youtube.com/watch?v=123',
      },
    ];
    const apiKey = 'test-api-key';
  ...
  it('should handle complex multimodal context (2 text + 2 YouTube)', async () => {
    ...
    const context = [
      { type: 'text' as const, text: 'Text Context 1' },
      {
        type: 'file_data' as const,
        mimeType: 'video/mp4',
        fileUri: 'https://youtube.com/v1',
      },
      { type: 'text' as const, text: 'Text Context 2' },
      {
        type: 'file_data' as const,
        mimeType: 'video/mp4',
        fileUri: 'https://youtube.com/v2',
      },
    ];
    const apiKey = 'test-api-key';
    ...
    expect(body.contents[0].parts[1]).toEqual({
      file_data: { mime_type: 'video/mp4', file_uri: 'https://youtube.com/v1' },
    });
    expect(body.contents[0].parts[2]).toEqual({ text: 'Text Context 2' });
    expect(body.contents[0].parts[3]).toEqual({
      file_data: { mime_type: 'video/mp4', file_uri: 'https://youtube.com/v2' },
    });
